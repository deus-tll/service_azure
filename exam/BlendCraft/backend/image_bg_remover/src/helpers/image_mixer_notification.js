  ...
  return new Promise((resolve, reject) => {
    amqp.connect(RABBITMQ_CONNECTION_URI, {}, async (errorConnect, connection) => {
      ...
      await connection.createChannel(async (errorChannel, _channel) => {
        ...
        await _channel.assertQueue(RABBITMQ_QUEUE_IMAGE_MIXER, {}, (errorQueue) => {
          if (errorQueue) {
            console.error(errorQueue);
            reject(errorQueue);
          }

          console.debug(`\"${RABBITMQ_QUEUE_IMAGE_MIXER}\" queue has been asserted to send.`);
        });
...
const rabbitMQ_notification = async (craftedImage) => {
  try {
    ...
    channel.sendToQueue(RABBITMQ_QUEUE_IMAGE_MIXER, Buffer.from(JSON.stringify(craftedImage)));
  } catch (error) {
